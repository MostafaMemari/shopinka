...
export default function FinalizeStickerDrawer({ isOpen, onClose }: FinalizeStickerDrawerProps) {
  ...
  return (
    <MobileDrawer open={isOpen} onOpenChange={onClose} showClose={false} className="max-w-[500px] m-auto">
      <div className="relative p-2 border-b min-h-[120px] flex items-center justify-center">
        {width && (
          <div className={`${dimensionLineClasses} top-0 left-1/2 mt-1 -translate-x-1/2 flex items-center`}>
        ...
        {width && <div className="absolute left-0 right-0 top-2 mt-1 border-b border-dashed border-red-300 pointer-events-none"></div>}
        ...
        <div
          ...
          className="relative z-0 text-center p-2"
        >
          {text}
        </div>
      </div>
