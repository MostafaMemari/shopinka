...
export default function CustomStickerDialog({ open, onOpenChange, thumbnail, customStickerValues }: CustomStickerDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md gap-6 overflow-hidden">
        <DialogHeader>
        ...
        <div className="space-y-4">
          <p className="text-xs font-bold text-muted-foreground mr-1">جزئیات متون و ابعاد:</p>
          <div className="grid gap-4">
            {lines.map((line) => (
              <div
                key={line.lineNumber}
                className="group relative rounded-xl border bg-card p-4 pt-5 transition-colors hover:border-primary/40"
              >
                  ...
                  <div className="flex items-center gap-4 pt-2 border-t border-dashed">
                    <div className="flex items-center gap-1.5 text-muted-foreground">
                      ...
                      <span className="text-xs font-medium text-text">
                        {line.width} <small>سانتی‌متر</small>
                      </span>
                    </div>

                    <div className="flex items-center gap-1.5 text-muted-foreground">
                      <MoveVertical className="w-4 h-4 text-green-500" />
                      <span className="text-[11px]">ارتفاع:</span>
                      <span className="text-xs font-medium text-text">
                        {line.height} <small>سانتی‌متر</small>
                      </span>
                    </div>
                  </div>
                </div>
